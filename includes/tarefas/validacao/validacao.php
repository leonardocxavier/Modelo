<?php
    ...
    */
    ...
    $sqlpending="SELECT *FROM `tasks` where `id_empresa`='$idempresa' and `id_projeto`='$idproj' and `status`='V'";
    ...
    while ($row = $resppending->fetch()){
        ...
        $itens_pending .= ' <div id="drag'.$row['id_task'].'" draggable="true" class="card card-info card-outline">
                                <div class="card-header">
                                    ...
                                    <a class="btn btn-tool btn-link" data-toggle="collapse" href="#collapse_'.$row['id_task'].'" role="button" aria-expanded="false" aria-controls="collapse_'.$row['id_task'].'">
                                        <i class="fas fa-minus"></i>
                                    </a>
                                    <a href="javascript:;" class="btn btn-tool">
                                    <i class="fas fa-trash"></i>
                                    </a>
                                    </div>
                                </div>
                                <div class="collapse" id="collapse_'.$row['id_task'].'">
                                    <div class="card card-body">
                                       '.htmlentities($row['descricao']).'
                                    </div>
                                </div>
                            </div>';
} 
